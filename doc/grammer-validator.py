...
##   - body that has optional consecutive variables such as V ::= [ Va Vb ], where V, Va, Vb are variables
##   - variable whose any body does not share first elements with others (then, the grammer is called LL1 grammer)
...
## TODO: runtime
TERMINALS = ['Identifier', "UnsignedInteger", "String"]
...
def validate_grammer():
    ...
    for head, bodies in grammer.items():
        for body in bodies:
            body = drop_terminals(body)
            optional_parts = re.findall(r'\[.*?\]|\{.*?\}', body)
    ...
    for head, bodies in grammer.items():
        ...
        first_set_list = []
        for body in bodies:
            body_head = extract_head_from_body(body)
            first_set_list.append(first(grammer, body_head))
        universe = ft.reduce(set.union, first_set_list, set())
        inter_set = ft.reduce(set.intersection, first_set_list, universe)
        if inter_set != set():
...
def first(grammer: dict, head: str) -> set[str]:
    if re.match(r'".*?"', head) or head in TERMINALS:
        return set([head])
    bodies = grammer[head]
    first_set = set()
    for body in bodies:
        body_head = extract_head_from_body(body)
        first_set = first_set.union(first(grammer, body_head))
    return first_set
    
def extract_head_from_body(body: str) -> str:
